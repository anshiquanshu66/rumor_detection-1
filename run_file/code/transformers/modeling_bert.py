...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(1024*3+768, config.num_labels)
    ...
    def forward(
        ...
        sequence_output1 = outputs[0]#32,150,768
        output1, hidden1 = self.gru(sequence_output1)
        hidden1 = hidden1.permute(1, 0, 2).reshape(input_ids.size(0), -1).contiguous()
        hidden1 = self.dropout(hidden1)

        sequence_output2 = outputs[2][-2]#32,150,768
        output2, hidden2 = self.gru(sequence_output2)
        hidden2 = hidden2.permute(1, 0, 2).reshape(input_ids.size(0), -1).contiguous()
        hidden2 = self.dropout(hidden2)


        sequence_output3 = outputs[2][-3]#32,150,768
        output3, hidden3 = self.gru(sequence_output3)
        hidden3 = hidden3.permute(1, 0, 2).reshape(input_ids.size(0), -1).contiguous()
        ...
        cat = torch.cat((pooler_out, hidden1, hidden2, hidden3), 1)
